...
void PriorityQueue<T>::remove(T element) 
{
  ...
  if(itr == locations.end())
  {
    ...
    std::cout << "ERROR! Element not found in priority queue" << std::endl;
    return;
  }
  //element for sure in queue
  int index = itr->second;
  locations.erase(m_heap[index]);
  if(index == 0)
  {
    m_heap[index] = m_heap.back();
    locations[m_heap[index]] = 0;
    m_heap.pop_back();
    this->percolate_down(0);
  }
  else if(index == m_heap.size()-1)
  {
    m_heap.pop_back();
  }
  else
...
void PriorityQueue<T>::percolate_up(int i) 
{
  ...
  while(i >= 1)
  {
    if(m_heap[i]->getPriorityValue() <= m_heap[i-1]->getPriorityValue())
    {
      //swaps 
      T lessThan = m_heap[i];
      T moreThan = m_heap[i-1];
      T temp;
      locations[moreThan] = i;
      locations[lessThan] = i-1;
      temp = m_heap[i-1];
      m_heap[i-1] = m_heap[i];
      m_heap[i] = temp;
      i--;
...
void PriorityQueue<T>::percolate_down(int i) 
{
  ...
  while(i <= m_heap.size()-2)
  {
    if(m_heap[i]->getPriorityValue() > m_heap[i+1]->getPriorityValue())
    {
      //swaps 
      T lessThan = m_heap[i+1];
      T moreThan = m_heap[i];
      T temp;
      locations[moreThan] = i+1;
      locations[lessThan] = i;
      temp = m_heap[i+1];
      m_heap[i+1] = m_heap[i];
      m_heap[i] = temp;
      i++;
